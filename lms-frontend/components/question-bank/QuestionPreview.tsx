    ...
    const renderContent = () => {
        switch (form.type) {
            case 'MCQ':
                return (
                    <div className="space-y-6">
                        <p className="text-slate-900 text-xl font-bold mb-6">{form.questionText}</p>
                        <div className="grid gap-4">
                            {(form.options || []).map((opt: string, i: number) => opt && (
                                <div key={i} className={`p-5 rounded-2x border-2 transition-all ${form.correctAnswer === opt
                            ...
                            <p className="text-slate-800 text-xl leading-relaxed">
                                {(form.questionText || '').split('[BLANK]').map((part: string, i: number, arr: any[]) => (
                                    <React.Fragment key={i}>
                                        {part}
                                        {i < arr.length - 1 && (
                                            <span className="inline-block min-w-[140px] border-b-3 border-primary-500 mx-2 text-primary-600 text-center font-black">
            ...
            case 'MQ':
                ...
                return (
                    <div className="space-y-6">
                        <p className="text-slate-900 text-xl font-bold">{form.questionText}</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div className="space-y-4">
                                ...
                                {(form.matchingPairs || []).map((p: any, i: number) => (
                                    <div key={i} className="p-5 bg-white border border-gray-100 rounded-2xl text-slate-800 flex items-center gap-4 shadow-sm hover:border-primary-200 transition-all">
                                        <span className="w-8 h-8 bg-gray-50 rounded-xl flex items-center justify-center text-xs font-bold text-slate-500">{i + 1}</span>
                            ...
                            <div className="space-y-4">
                                ...
                                {shuffledRights.map((r: string, i: number) => (
                                    <div key={i} className="p-5 bg-white border border-gray-100 rounded-2xl text-slate-800 flex items-center gap-4 shadow-sm hover:border-primary-200 transition-all">
                                        <span className="w-8 h-8 bg-gray-50 rounded-xl flex items-center justify-center text-xs font-bold text-slate-500">{String.fromCharCode(65 + i)}</span>
            ...
            case 'JC':
                return (
                    <div className="space-y-6">
                        <p className="text-slate-900 text-xl font-semibold mb-4">{form.questionText}</p>
                            ...
                            <div className="space-y-3">
                                {(form.jumbledStatements || []).map((s: string, i: number) => (
                                    <div key={i} className="flex gap-4 items-start group">
                                        <div className="mt-2 w-2 h-2 rounded-full bg-primary-400 group-hover:bg-primary-600 transition-colors" />
                                        <span className="text-slate-700 text-sm whitespace-pre-wrap">{s}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-white p-8 rounded-2xl border-l-4 border-primary-500 shadow-sm border-y border-r border-gray-100">
                            <div className="flex justify-between items-start gap-4 mb-4">
                                <h3 className="text-2xl font-bold text-slate-900 mb-2">{form.topicNames} Challenge</h3>
                                ...
                                <div>
                                    ...
                                    {(form.testCases || []).map((tc: any, i: number) => i === 0 && (
                                        <div key={i} className="p-4 bg-gray-50/50 rounded-xl border border-gray-100 space-y-3">
                                            ...
                                            <pre className="text-xs text-indigo-600 bg-white p-3 rounded-lg border border-gray-100">{tc.input}</pre>
                                            <p className="text-[10px] text-gray-500 font-bold uppercase mt-2">Sample Output</p>
                                            <pre className="text-xs text-emerald-600 bg-white p-3 rounded-lg border border-gray-100">{tc.output}</pre>
                                        </div>
                                    ))}
                                </div>
                            </div>
