...
import { IsString, IsEnum, IsOptional, IsArray, IsInt, IsDateString } from 'class-validator';
...
export class ContestController {
    ...
    @Get()
    getAll() {
        return this.contestRepo.find({ relations: ['createdBy'], order: { createdAt: 'DESC' } });
    }

    @Get(':id')
    getOne(@Param('id', ParseIntPipe) id: number) {
        return this.contestRepo.findOne({ where: { id }, relations: ['createdBy'] });
    }
    ...
    @Put(':id')
    async update(@Param('id', ParseIntPipe) id: number, @Body() dto: Partial<CreateContestDto>) {
