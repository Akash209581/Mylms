...
import { Question, QuestionType, Difficulty } from '../entities/question.entity';
import { JwtAuthGuard } from '../common/jwt.guard';
import { RolesGuard } from '../common/roles.guard';
import { Roles } from '../common/roles.decorator';
import { UserRole } from '../entities/user.entity';
import { IsString, IsEnum, IsOptional, IsArray, IsNumber, IsInt } from 'class-validator';
...
class CreateQuestionDto {
    ...
    @IsArray() @IsOptional() testCases?: { input: string; output: string; explanation?: string }[];
...
export class QuestionBankController {
    ...
    async getAll(
        ...
        if (topic) qb.andWhere('q.topicNames ILIKE :topic', { topic: `%${topic}%` });
    ...
    @Put(':id')
    async update(@Param('id', ParseIntPipe) id: number, @Body() dto: Partial<CreateQuestionDto>) {
